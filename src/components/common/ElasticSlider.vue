  ...
  <div :class="`flex flex-col items-center justify-center gap-4 w-48 ${className}`">
    ...
    >
      ...
      >
        ...
        >
          <div class="elastic-slider__bar">
            <div class="elastic-slider__bar-fill" :style="{ width: `${rangePercentage}%` }" />
          </div>
        </div>
      </div>
    ...
    <p class="elastic-slider__value">{{ Math.round(value) }}</p>
  </div>
</template>

<script setup lang="ts">
import { ref, computed, watch, onMounted, onUnmounted, type Component, useTemplateRef } from 'vue';
