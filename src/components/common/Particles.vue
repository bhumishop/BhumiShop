...
const initParticles = () => {
  currentContainer = containerRef.value;
  ...
  resizeHandler = () => {
    ...
    const parentWidth = currentContainer.parentElement?.offsetWidth || currentContainer.offsetWidth || window.innerWidth;
    const parentHeight = currentContainer.parentElement?.offsetHeight || currentContainer.offsetHeight || window.innerHeight;
  ...
  const update = (t: number) => {
    ...
    if (program) {
      // Only update time uniform - others are static
    ...
    if (particles) {
      ...
      } else if (particles.position.x !== 0 || particles.position.y !== 0) {
  ...
  return () => {
    ...
    if (props.moveParticlesOnHover && currentContainer) {
    ...
    if (currentContainer && renderer && currentContainer.contains(gl.canvas)) {
