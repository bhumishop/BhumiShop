...
const borderBg = computed(() => {
  const bg = meshGradients.value;
  return bg.map(g => `${g} border-box`);
});
const fillBg = computed(() => {
  const bg = meshGradients.value;
  return bg.map(g => `${g} padding-box`);
});
const angleDeg = computed(() => `${cursorAngle.value.toFixed(3)}deg`);

// Precompute conic gradient masks for border
const borderMaskBase = computed(() => {
  const cone = props.coneSpread;
  const coneOuter = 100 - cone;
  const transInner = cone + 15;
  const transOuter = coneOuter - 15;
  return `conic-gradient(from ${angleDeg.value} at center, black ${cone}%, transparent ${transInner}%, transparent ${transOuter}%, black ${coneOuter}%)`;
});

// Precompute conic gradient masks for fill
const fillMaskBase = computed(() => {
  const angle = angleDeg.value;
  return `conic-gradient(from ${angle} at center, transparent 5%, black 15%, black 85%, transparent 95%)`;
});

// Precompute outer glow mask
const glowMaskBase = computed(() => {
  const angle = angleDeg.value;
  return `conic-gradient(from ${angle} at center, black 2.5%, transparent 10%, transparent 90%, black 97.5%)`;
});

const borderMaskImage = computed(() => borderMaskBase.value);
const borderMaskWebkit = computed(() => borderMaskBase.value);
const fillMaskImage = computed(() => fillMaskBase.value);
const fillMaskWebkit = computed(() => fillMaskBase.value);
const glowMaskImage = computed(() => glowMaskBase.value);
const glowMaskWebkit = computed(() => glowMaskBase.value);

// Precompute border shadow
  ...
  >
    ...
    <div
      ...
      :style="{
        ...
        WebkitMaskImage: borderMaskWebkit,
    ...
    <span
      ...
      :style="{
        ...
        maskImage: glowMaskImage,
        WebkitMaskImage: glowMaskWebkit,
        opacity: glowOpacity,
