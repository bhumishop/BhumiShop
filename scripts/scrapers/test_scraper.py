...
# Add this directory to path so we can import umapenca/uiclap
...
# Stores that are temporarily unreachable (reported, not failed)
SKIPPED_STORES = set()
FAILED_STORES = set()


def store_reachable(url: str) -> tuple:
    """Health-check a store URL. Returns (ok, detail)."""
    import requests
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "Mozilla/5.0 (compatible; BhumiShopScraper/1.0)"},
        )
...
def test_store_scraper(store_name: str, store_config: dict) -> list:
    ...
    ok, detail = store_reachable(store_config["url"])
    if not ok:
        print(f"  [SKIP] {store_name} unreachable ({detail}) — pending upstream fix")
        SKIPPED_STORES.add(store_name)
        return []
...
def test_products_json_files():
    ...
    for store_name in STORES:
        json_path = os.path.join(base_dir, store_name, "output", "products.json")
...
def main():
    ...
    if FAILED_STORES:
        print(f"\n[FAIL] Stores failing: {', '.join(sorted(FAILED_STORES))}")
        return 1

    if SKIPPED_STORES:
        print(f"[SKIP] Pending upstream fix: {', '.join(sorted(SKIPPED_STORES))}")

    if all_products:
        print(f"\n[SUCCESS] Scraper working for {len(STORES) - len(SKIPPED_STORES)} store(s)")
        return 0
    else:
